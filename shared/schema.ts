...
export const echoMemories = pgTable("echo_memories", {
  ...
  sourceType: text("source_type").notNull(), // "text" | "voice" | "screenshot" | "photo"
  rawFileUrl: text("raw_file_url"),
  tags: text("tags").notNull().default("[]"), // JSON array of strings
  people: text("people").notNull().default("[]"), // JSON array of person names
  emotionalWeight: text("emotional_weight").notNull().default("medium"), // "low" | "medium" | "high"
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  resurface: text("resurface").notNull().default("false"), // queued for "Today You Forgot"
  resurfacedAt: text("resurfaced_at"),
});

export const insertEchoMemorySchema = createInsertSchema(echoMemories).pick({
  userId: true,
  content: true,
  sourceType: true,
  rawFileUrl: true,
...
export type InsertEchoMemory = z.infer<typeof insertEchoMemorySchema>;
export type EchoMemory = typeof echoMemories.$inferSelect;

// People known to the user
export const echoPeople = pgTable("echo_people", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  name: text("name").notNull(),
  aliases: text("aliases").notNull().default("[]"), // JSON array
  relationshipType: text("relationship_type").notNull().default("other"), // "spouse" | "child" | "parent" | "friend" | "colleague" | "other"
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
});

export const insertEchoPersonSchema = createInsertSchema(echoPeople).pick({
  userId: true,
  name: true,
  aliases: true,
...
export type InsertEchoPerson = z.infer<typeof insertEchoPersonSchema>;
export type EchoPerson = typeof echoPeople.$inferSelect;
