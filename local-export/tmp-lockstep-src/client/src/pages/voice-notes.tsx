...
export function VoiceNotesPage() {
  ...
  const handleUploadComplete = async (voiceNoteId: string) => {
    ...
    try {
      ...
      if (extractError) {
        throw new Error(extractError.message || 'Intent extraction failed');
      }

      if (!data?.extractedIntent) {
        throw new Error('No intent data returned from extraction');
      }

      setExtractedIntent(data.extractedIntent);
      setSuccessMessage('Intent extracted. Review below and create a commitment.');
      analytics.track('intent_extracted', {
        emotion: data.extractedIntent.emotion,
        confidence: data.extractedIntent.confidenceScore,
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Extraction failed';
      console.error('Intent extraction error:', err);
      setError(`${message}. Check that API keys are configured in Supabase.`);
    } finally {
