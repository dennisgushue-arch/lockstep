...
if (!res.ok) throw new Error((json as any)?.error || `fail_commitment ${res.status}`);
...
return new Response(JSON.stringify({ ok: true, scanned: rows?.length ?? 0, ok_count: ok, failed, errors }), {
status: 200,
headers: { "Content-Type": "application/json" },
});
} catch (e) {
