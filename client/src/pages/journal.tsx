    ...
    try {
      ...
      if (combinedText) {
        try {
          ...
          // Non-blocking - still save check-in even if detection fails
        }
      }
      ...
      setNote("");
      setProof("");
      analytics.track('journal_checkin_saved', {
        commitmentId,
        hasNote: !!note.trim(),
        hasProof: !!proof.trim(),
      });
      ...
      toast({
        title: "Save failed",
        description: error instanceof Error ? error.message : "Unable to save check-in. Try again.",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
